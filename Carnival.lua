...
local dahood_ids = {2788229376, 16033173781, 7213786345, 103539521748777, 104408328640771, 104596910487466, 15186202290}
...
--
-- // Crosshair Handler \\ --
local Cursor = loadstring(game:HttpGet('https://pastebin.com/raw/bYPanGvP', true))() do
...
local Script = {
	...
	Visuals = {
	    SelfESP = {
		    Chams = {
			ForecefieldBodyEnabled = true,
			ForcefieldToolsEnabled = true,
			ForcefieldHatsEnabled = true,
			ForcefieldBodyColor = MainColor,
			ForcefieldToolsColor = MainColor,
			ForecdfieldHatsColor = MainColor
		},
		Trail = {
		    Enabled = true,
			LifeTime = 2,
...
---
function Carnival:Render(type,properties)
...
--
function Carnival:Instance(type,properties)
...
--
local SilentAimFOVCircle = Carnival:Render("Circle", {
...
--
local TriggerBotDot = Carnival:Render("Circle", {
    Visible = false,
    Color = MainColor,
    Radius = 20,
    Thickness = 1
...
--
local Line = Carnival:Render("Line", {
    Visible = false,
    Color = MainColor,
    Thickness = 2,
    Transparency = 0.5,
...
--
local Dot = Carnival:Render("Circle", {
    Visible = false,
    Filled = true,
    Color = MainColor,
})
--
local TargetChams = Carnival:Instance("Highlight", {
    Parent = nil,
    FillColor = MainColor,
    OutlineColor = MainColor,
})
--
local CFrameDesyncDot  = Carnival:Render("Circle", {
    Visible = false,
    Filled = true
})

local CFrameDesyncTracer  = Carnival:Render("Line", {
    Visible = false,
    Color = MainColor,
    Thickness = 2
})
...
--
function Carnival:RandomVectorThree(randomization)
...
--
function Carnival:BehindWall(player)
...
--
function Carnival:RecalculatePlayerVelocity(player)
...
--
function Carnival:GetClosestBodyPart(Character)
...
--
function Carnival:GetClosestPoint(part)
...
--
function Carnival:GetClosestPlayerToCursor(Huh)
    closestDist = Huh
    closestTarget = nil
    for _, v in pairs(Players:GetPlayers()) do
        if v ~= LocalPlayer and v.Character and v.Character:FindFirstChild("Humanoid") and v.Character.Humanoid.Health > 0 then
            if Script.Checks.Enabled and (
                ...
                (Script.Checks.Wall and Carnival:BehindWall(v))
            ) then 
                continue 
            end
...
--
function Carnival:CalculateResolverOfset(Player)
    ...
    if Script.Resolver.Enabled then
        ...
        if method == "Recalculate" then
            offset = Carnival:RecalculatePlayerVelocity(player) * prediction
...
--
function Carnival:AimViewerCalculator()
...
--
function Carnival:CalculateAimAssistPredictedPosition()
    ...
    if Script.Checks.Enabled and (
        ...
        Script.Checks.Wall and Carnival:BehindWall(Target)
    ) then
        return
    end
...
--
function Carnival:TrailCharacter(Bool)
...
--
function Carnival:CloneCharacter(Player,transparency,color,material,duration)
...
--
function Carnival:CreateBeam(from,to,color_1,color_2,duration,fade_enabled,fade_duration)
    local tween
    local total_time = 0

    local main_part = Carnival:Instance("Part",{
        Parent = workspace,
    ...
    local part0 = Carnival:Instance("Part",{
        Parent = main_part,
        Size = Vector3.new(0,0,0),
        Massless = true,
        Transparency = 1,
    ...
    local part1 = Carnival:Instance("Part",{
        Parent = main_part,
        Size = Vector3.new(0,0,0),
        Massless = true,
        Transparency = 1,
    ...
    local attachment0 = Carnival:Instance("Attachment",{
        Parent = part0
    })

    local attachment1 = Carnival:Instance("Attachment",{
        Parent = part1
    })

    local beam = Carnival:Instance("Beam",{
    ...
    if fade_enabled then
        tween = RunService.PostSimulation:Connect(function(delta_time) --// credits to jake
...
--
function Carnival:CreateImpact(color,size,fade_enabled,fade_duration,duration,position)
    local impact = Carnival:Instance("Part",{
        CanCollide = false,
    ...
    local outline = Carnival:Instance("SelectionBox",{ --// credits to mavyr
        LineThickness = 0.01,
        Color3 = color,
...
--
function Carnival:CreateDrawingBeam(v1,v2)
...
RunService.PostSimulation:Connect(function()
    local TriggerBotTarget = Carnival:GetClosestPlayerToCursor(Script.TriggerBot.Range)
...
RunService.PostSimulation:Connect(function()
    if Target and Target.Character then
        if Script.TargetAim.HitPart.ClosestPart then
            local hitPartName = tostring(Carnival:GetClosestBodyPart(Target.Character))
            ...
            elseif Script.TargetAim.HitPart.ClosestMode == "Nearest Point" then
                InnalillahiMichaelJackson.Locals.HitPart = hitPartName
                InnalillahiMichaelJackson.Locals.AimPoint = Carnival:GetClosestPoint(Target.Character[hitPartName])
            end
        else
        ...
        if Script.Rage.Killbot.Enabled and (not Script.Rage.Killbot.BypassDC or (Script.Rage.Killbot.BypassDC and Target.Character.HumanoidRootPart.Position.Y >= -10000)) then
            ...
            if Script.Rage.Killbot.Type == "Orbit" then
                ...
                strafe = Target.Character.HumanoidRootPart.CFrame * CFrame.new(Carnival:RandomVectorThree(Script.Rage.Killbot.Randomization))
            end
...
RunService.PostSimulation:Connect(function()
   if Script.TargetAim.Enabled and Script.TargetAim.Type == "Silent Aim" then
        Target = Carnival:GetClosestPlayerToCursor(Script.Visuals.FoV.Size+15)
    end
    --
    ...
    --
    Carnival:CalculateAimAssistPredictedPosition()
end)
...
RunService.PostSimulation:Connect(function()
   ...
   if (Script.AntiAim.CSync.Enabled or Script.AntiAim.CSync.VoidSpam or Script.AntiAim.CSync.DestroyCheaters) and LocalPlayer.Character then
        ...
        local cframe_position = {
            ...
            ["Random"] = attach.CFrame + Carnival:RandomVectorThree(Script.AntiAim.CSync.Randomize.Value),
            ...
            if PlayerChecks.PlayerGun then
                ...
                if Script.Visuals.BulletTracers.Enabled and Script.Visuals.BulletTracers.Mode == "Global" then
                    ...
                    if Script.Visuals.BulletTracers.Type == "Beam" then
                        Carnival:CreateBeam(BulletRay.Position, Attachment1.WorldCFrame.Position, Script.Visuals.BulletTracers.Color, Script.Visuals.BulletTracers.Color, Script.Visuals.BulletTracers.Duration, Script.Visuals.BulletTracers.Fade, Script.Visuals.BulletTracers.FadeDuration)
                    else
                        Carnival:CreateDrawingBeam(BulletRay.Position, Attachment1.WorldCFrame.Position)
                    end
                end
                
                if Script.Visuals.BulletImpacts.Enabled then
                    Carnival:CreateImpact(Script.Visuals.BulletImpacts.Color, Script.Visuals.BulletImpacts.Width, Script.Visuals.BulletImpacts.Fade, Script.Visuals.BulletImpacts.FadeDuration, Script.Visuals.BulletImpacts.Duration, Attachment1.WorldPosition)
                end                
                ...
                if Script.Visuals.Hit_Detection.Enabled then
                    ...
                    if Target then
                        if Script.Visuals.Hit_Detection.Notify then
                            Notifications:Notify('[<font color="#'..tostring(MainColor:ToHex())..'">Carnival</font>] Hitted <font color="#'..tostring(MainColor:ToHex())..'">'..tostring(Target.DisplayName)..'</font> in the <font color="#'..tostring(MainColor:ToHex())..'">'..tostring(HitPart)..'</font>', 5)
                        end

                        if Script.Visuals.Hit_Detection.Clone then
                            Carnival:CloneCharacter(Target,0.5,MainColor,"Neon",5)
                        end
                        
                        if Script.Visuals.Hit_Detection.Sound then
                            local Sound = Carnival:Instance("Sound", {
            ...
            if tool and Script.Visuals.BulletTracers.Enabled and Script.Visuals.BulletTracers.Mode == "Local" and tool:FindFirstChild("Ammo") then
                getgenv().Loop = tool.Ammo.Changed:Connect(function()
                    if tool.Ammo.Value ~= tool.MaxAmmo.Value then
                        if Script.Visuals.BulletTracers.Type == "Beam" then
                            Carnival:CreateBeam(tool.Handle.Position, LocalPlayer.Character.BodyEffects.MousePos.Value, Script.Visuals.BulletTracers.Color, Script.Visuals.BulletTracers.Color, Script.Visuals.BulletTracers.Duration, Script.Visuals.BulletTracers.Fade, Script.Visuals.BulletTracers.FadeDuration)
                       else
                            Carnival:CreateDrawingBeam(tool.Handle.Position, LocalPlayer.Character.BodyEffects.MousePos.Value)
                       end
                        --
                        RunService.RenderStepped:Wait()
                        --
...
local Window = Library:CreateWindow({
    Title = 'Carnival<font color="rgb(255,105,180)">.lua</font>',
    Center = true,
    AutoShow = true,
    TabPadding = 8,
    MenuFadeTime = 0.2
          ...
          Callback = function()
            ...
            if combatSettings.TargetAim.Enabled then
                if string.lower(combatSettings.TargetAim.Type) == "target aim" then
                    if not Target then
                        Target = Carnival:GetClosestPlayerToCursor(math.huge)
                        Notifications:Notify('[<font color="#' .. MainColor:ToHex() .. '">Carnival</font>] Target: ' .. tostring(Target), 1.5)
                    else
                        Target = nil
                        Notifications:Notify('[<font color="#' .. tostring(MainColor:ToHex()) .. '">Carnival</font>] Unlocked', 1.5)
                    end
                end
            end
        end
    ...
    Sections.Combat.Main:AddButton("Load Button", function()
        ...
        TextButton.MouseButton1Click:Connect(function()
            ...
            if combatSettings.TargetAim.Enabled then
                if string.lower(combatSettings.TargetAim.Type) == "target aim" then
                    if not Target then
                        TextButton.Text = "Carnival ON"
                        Target = Carnival:GetClosestPlayerToCursor(math.huge)
                        Notifications:Notify('[<font color="#' .. MainColor:ToHex() .. '">Carnival</font>] Target: ' .. tostring(Target), 1.5)
                    else
                        Target = nil
                        TextButton.Text = "Carnival OFF"
                        Notifications:Notify('[<font color="#' .. tostring(MainColor:ToHex()) .. '">Carnival</font>] Unlocked', 1.5)
                    end
                end
            end
...
-- UI Settings
Library:SetWatermark('Carnival.lua | '..os.date("%B %d %Y").." | 1.0.1")
...
ThemeManager:SetFolder("Carnival")

SaveManager:SetLibrary(Library)
SaveManager:SetFolder("Carnival/RaidAwareness")
